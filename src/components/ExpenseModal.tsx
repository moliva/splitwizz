...
export const ExpenseModal = (props: ExpenseModalProps) => {
  ...
  const checkConfirm = () => {
    const newValue =
      (descriptionRef?.value?.length ?? 0) === 0 ||
      (amountRef?.value ?? '') === '' ||
      (dateRef?.value ?? '') === '' ||
  ...
  return (
    <div class={editGroupStyles.modal}>
      <div class={editGroupStyles['modal-content']} style={{ 'max-width': '400px' }}>
        <label class={editGroupStyles['modal-title']}>
          New expense in <span class={styles['group-name']}>{group.name}</span>
        </label>
        ...
        <div style={{ display: 'inline-flex', 'margin-bottom': '20px' }}>
          <select class={styles['currency-select']} ref={currencyRef} value={group?.default_currency_id}>
        ...
        <div style={{ display: 'flex', 'flex-direction': 'column' }}>
          ...
          <UserSelect
            ...
            selectionLimit={1}
          />
          <label class={styles['user-select-label']} style={{ 'margin-top': '10px' }}>
        ...
        <div class={editGroupStyles['modal-controls']}>
          ...
          <button class={`${appStyles.button} ${appStyles.secondary}`} onClick={onDiscard}>
            Discard
          </button>
        </div>
      </div>
